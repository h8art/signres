<template lang="pug">
  .cont 
    .head
      ...
      .btn VIEW
    h3 Left a signature {{endPointsSum}}/{{endPoints.length}}
    .ready 
      .ready-item(v-for='item in endPoints')
        img(:src='getAvatar(item.address)')
        span {{item.address}}
        .yes(v-if='item.isReady')
        .no(v-else)
</template>
...
export default {
  ...
  created(){
    var documents
    axios.get('https://testnodes.wavesnodes.com/addresses/data/3N4keZuu6hGfJ3WnYfgJguPv6TCsntDV3ds').then(response=>{
